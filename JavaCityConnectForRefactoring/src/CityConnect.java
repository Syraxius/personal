...
public class CityConnect {
	...
	private static final String MESSAGE_UNRECOGNIZED_TYPE = "Unrecognized command type";
	private static final String MESSAGE_NULL_ENDPOINTS = "Route end points cannot be null";
	private static final String COMMAND_TYPE_NULL = "command type string cannot be null!";
	...
	private static void runUntilUserExits() {
		while (true) {
	...
	public static String executeCommand(String userCommand) {
		...
		boolean isUserCommandBlank = userCommand.trim().equals("");
		if (isUserCommandBlank) {
			return errorMessage(userCommand);
		}
		...
		case EXIT:
			System.exit(0);
		default:
			throw new Error(MESSAGE_UNRECOGNIZED_TYPE);
		}
		/*
		 * ==============NOTE TO STUDENTS======================================
	...
	private static String errorMessage(String userCommand) {
	...
	private static COMMAND_TYPE determineCommandType(String commandTypeString) {
		...
		if (commandTypeString == null) {
			throw new Error(COMMAND_TYPE_NULL);
		}
	...
	private static String getDistance(String userCommand) {
		...
		String withoutFirstWord = removeFirstWord(userCommand);
		String[] parameters = splitParameters(withoutFirstWord);

		boolean isWrongParameterCount = parameters.length < PARAM_SIZE_FOR_GET_DISTANCE;

		if (isWrongParameterCount) {
			return errorMessage(userCommand);
		}
	...
	private static int getPositionOfExistingRoute(String newStartLocation,
		...
		for (int i = 0; i < route.length; i++) {
			...
			boolean noMoreRoutes = existing_start_location == null;

			if (noMoreRoutes) {
				return NOT_FOUND;
			}

			boolean routeFound = sameRoute(existing_start_location,
	...
	private static int location(String newStartLocation, String newEndLocation) {
		...
		for (int i = 0; i < route.length; i++) {
			...
			boolean isSlotEmpty = existingStartLocation == null;
			if (isSlotEmpty) {
				return i;
			}

			boolean isSameRoute = sameRoute(existingStartLocation,
					existingEndLocation, newStartLocation, newEndLocation);
			if (isSameRoute) {
				return i;
			}
		}
	...
	private static boolean sameRoute(String startLocation1,
		...
		boolean isMissingStart1 = startLocation1 == null;
		boolean isMissingEnd1 = endLocation1 == null;
		boolean isMissingStart2 = startLocation2 == null;
		boolean isMissingEnd2 = endLocation2 == null;

		boolean isMissingEndpoints = isMissingStart1 || isMissingEnd1
				&& isMissingStart2 || isMissingEnd2;

		if (isMissingEndpoints) {
			throw new Error(MESSAGE_NULL_ENDPOINTS);
		}

		boolean isSameStartForward = startLocation1
				.equalsIgnoreCase(startLocation2);
		boolean isSameEndForward = endLocation1.equalsIgnoreCase(endLocation2);
		boolean isSameStartReversed = startLocation1
				.equalsIgnoreCase(endLocation2);
		boolean isSameEndReversed = endLocation1
				.equalsIgnoreCase(startLocation2);

		boolean isSameRoute = (isSameStartForward && isSameEndForward)
				|| (isSameStartReversed && isSameEndReversed);
